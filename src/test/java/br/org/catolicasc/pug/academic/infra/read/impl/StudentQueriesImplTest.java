...
@QuarkusTest
class StudentQueriesImplTest extends BaseSearchTest {
  ...
  void shouldFindById() {
    var view = queries.findOptionalById(student.getAccountId());

    assertThat(view).isPresent();
    assertThat(view.get().accountId()).isEqualTo(student.getAccountId());
  }
  ...
  void shouldListViewsByAccountIds() {
    var list = queries.listViewsByAccountIds(List.of(student.getAccountId()));
    assertThat(list).hasSize(1);
    assertThat(list.get(0).accountId()).isEqualTo(student.getAccountId());
  }
  ...
  @Test
  @Transactional
  @DisplayName("Should verify all fields in student view")
  void shouldVerifyAllFieldsInView() {
    ...
    assertThat(view).isPresent();
    var sv = view.get();
    assertThat(sv.accountId()).isEqualTo(student.getAccountId());
    assertThat(sv.campus()).isEqualTo(student.getCampus());
    assertThat(sv.courseId()).isEqualTo(student.getCourseId());
    ...
    assertThat(sv.concluded()).isEqualTo(student.getCounterpartHours().getConcluded());
    assertThat(sv.startDate()).isEqualTo(student.getPeriod().getStartDate());
    assertThat(sv.dueDate()).isEqualTo(student.getPeriod().getDueDate());
  ...
  void shouldSearchByNameSuccess() throws Exception {
    ...
    assertThat(results).anyMatch(v -> v.accountId().equals(student.getAccountId()));
  }
