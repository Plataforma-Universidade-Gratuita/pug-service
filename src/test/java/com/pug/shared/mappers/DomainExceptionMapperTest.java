...
class DomainExceptionMapperTest {
    ...
    void buildDetailsAllBranchesCovered() throws Exception {
        ...
        assertEquals(
                ...
                details(mapper, ex(ENTITY_NOT_FOUND, "11222333000181")));
        ...
        assertEquals(
                Map.of("userRoleId", uid), details(mapper, ex(STAFF_NOT_FOUND, uid)));
        assertEquals(
                Map.of("userRoleId", uid), details(mapper, ex(STAFF_DUPLICATE_USER_ROLE_ID, uid)));

        // USER_DUPLICATE_CPF
        assertEquals(
                Map.of("cpf", "93541134780"),
                details(mapper, ex(USER_DUPLICATE_CPF, "93541134780")));
