 ...
 * (such as CPF uniqueness), and coordinates with the persistence layer to ensure the integrity
...
public interface UserService {
   ...
   * internal validations (e.g., CPF mathematical validation) are strictly applied before persistence.
   *
   ...
   * @throws com.pug.shared.exceptions.DuplicateResourceException if a user with the provided CPF already exists
   * @throws com.pug.shared.exceptions.AppValidationException     if the input data violates domain constraints
   *                                                              (e.g., blank name, malformed CPF)
   */
   ...
   * @throws com.pug.shared.exceptions.ResourceNotFoundException  if the user cannot be found in the repository
   * @throws com.pug.shared.exceptions.DuplicateResourceException if the updated CPF conflicts with an existing user
   ...
   * @return {@code true} if the user was successfully deleted, {@code false} if the ID is null
   ...
   * (e.g., loading an aggregate to mutate it or link it). For API responses,
   * use {@link UserReadService#getViewById(UUID)} instead.
   *
   ...
   * @throws com.pug.shared.exceptions.AppValidationException    if the user exists in the database but its stored state
   *                                                             currently violates strict domain invariants (data corruption)
   */
