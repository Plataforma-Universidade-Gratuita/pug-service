 ...
 * It enforces cross-cutting business rules (e.g., email uniqueness) and manages the complex
 * lifecycle relationship between an {@link Account} and its underlying {@link User}, automatically
 * provisioning or pruning user records to prevent orphans.
 */
public interface AccountService {
   ...
   * <p>
   * This method performs a cascading save. It checks if a {@link User} already exists for the
  ...
  /**
   * Updates an existing {@link Account} (and optionally its linked {@link User}) using the provided data.
   * <p>
   * This method applies partial updates. If user data is provided in the command, the update
   * is cascaded down to the underlying user aggregate.
   *
   ...
   * @throws com.pug.shared.exceptions.DuplicateResourceException if the updated email conflicts with an existing account
   ...
   * <p>
   * This operation enforces data hygiene. After the account is deleted, the service checks
   * if the underlying {@link User} has any other active accounts. If they do not, the user
   * is automatically deleted to prevent orphaned records.
   *
   ...
   * <p>
   * Similar to single deletion, this batch operation evaluates the underlying users post-deletion
   * and purges any users that have been orphaned by the removal of these accounts.
   *
   ...
   * For API responses, use {@link AccountReadService#getViewById(UUID)} instead.
   *
   * @param id the unique identifier (UUID) of the account
   * @return the fully reconstituted {@link Account} aggregate
   * @throws com.pug.shared.exceptions.ResourceNotFoundException if the account does not exist
   * @throws com.pug.shared.exceptions.AppValidationException    if the account exists but its stored state
