 ...
 * for administrative privileges. It orchestrates the complex lifecycle relationship between
...
public interface AdminService {
   ...
   * <p>
   * This method performs a cascading save. It delegates the creation of the underlying
   ...
   * @throws com.pug.shared.exceptions.DuplicateResourceException if the underlying account email already exists
   */
  Admin save(AdminCreateCommand cmd);

  /**
   * Updates an existing {@link Admin} and optionally its underlying account using the provided data.
   * <p>
   * This method applies partial updates. If account data is provided in the command,
   * the update is cascaded down to the underlying account aggregate.
   *
   * @param accountId the unique identifier of the Admin (which corresponds directly to the Account ID)
   ...
   * @throws com.pug.shared.exceptions.ResourceNotFoundException if the Admin or linked Account does not exist
   ...
   * <p>
   * This operation enforces data hygiene. After the admin privileges are successfully
   ...
   * For API responses, use {@link AdminReadService#getViewByAccountId(UUID)} instead.
   *
   ...
   * @throws com.pug.shared.exceptions.AppValidationException    if the Admin exists but its stored state
