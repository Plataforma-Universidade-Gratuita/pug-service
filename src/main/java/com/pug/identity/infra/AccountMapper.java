 ...
 * <p>
 * This mapper acts as an anti-corruption layer, ensuring that the pure Domain model ({@link Account})
 * does not leak into or depend upon the JPA Persistence model ({@link AccountEntity}) or the
 * Read/Query model ({@link AccountView}).
 */
public final class AccountMapper {
   ...
   * Domain Value Objects (e.g., {@link Email}, {@link AuditInfo}).
   *
  ...
  /**
   * Translates a pure Domain {@link Account} aggregate into a newly instantiated JPA {@link AccountEntity}.
   * <p>
   ...
   * @return a newly constructed JPA {@link AccountEntity}, or {@code null} if the input domain is null
   */
  ...
  /**
   * Updates an existing, attached JPA {@link AccountEntity} with the current state of a Domain {@link Account}.
   * <p>
   * This method allows the ORM to track changes for updates. The primary key (ID) and the
  ...
  /**
   * Projects an {@link AccountEntity} and its parent {@link UserEntity} into a consolidated {@link AccountView} DTO.
   * <p>
   * Used heavily by the query/read layer to provide a rich, nested structure ready
   * for JSON serialization without requiring complex domain logic or repeated database hits.
   *
