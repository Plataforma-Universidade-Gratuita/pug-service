...
public class AccountReadOnlyResource {
    ...
    public Response list() {
        List<AccountResponse> list =
                readService.listViews().stream().map(v -> AccountPresenter.toResponse(v, locale(), i18n)).toList();
    ...
    public Response listByCpf(@PathParam("cpf") @NotNull String cpfRaw) {
        ...
        List<AccountResponse> list =
                readService.listViewsByCpf(cpf).stream().map(v -> AccountPresenter.toResponse(v, locale(), i18n)).toList();
        return Response.ok(ApiEnvelope.ok(BulkCreateResult.of(list))).build();
    }
    ...
    public Response listByName(@QueryParam("q") String query) {
        ...
        List<AccountResponse> list =
                readService.search(query).stream().map(v -> AccountPresenter.toResponse(v, locale(), i18n)).toList();
        return Response.ok(ApiEnvelope.ok(BulkCreateResult.of(list))).build();
    }
}
