...
public class PersistenceExceptionMapper implements ExceptionMapper<PersistenceException> {
     ...
     *
     * <p>This mapper inspects the root cause of the PersistenceException to determine if it's a
     * known database constraint violation (like unique constraints) or other data-related issues,
     * and constructs a meaningful API error response accordingly.
     *
     ...
     *
     * <p>This method checks the name of the violated constraint against known patterns (both
     * Postgres default and custom annotation names) to determine the appropriate error code and
     * message to return. If the constraint is not recognized, it falls back to a generic data
     ...
     * @return A Response object with a conflict status and specific error details if recognized,
     * or a generic data integrity error if not.
     */
    private Response handleConstraintViolation(String constraintName, ConstraintViolationException ex) {
        ...
        if (isConstraint(constraintName, "cities_ibge_code_key", "uq_cities_ibge_code")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "ibgeCode", "City with this IBGE code already exists");
        }
        // --- PARTNER DOMAIN ---
        if (isConstraint(constraintName, "entities_cnpj_key", "uq_entities_cnpj")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "cnpj", "Entity with this CNPJ already exists");
        }
        // --- ACADEMIC DOMAIN ---
        if (isConstraint(constraintName, "schools_name_key", "uq_schools_name")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "name", "School with this name already exists");
        }
        if (isConstraint(constraintName, "courses_name_key", "uq_courses_name")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "name", "Course with this name already exists");
        }
        if (isConstraint(constraintName, "students_academic_registration_key", "uq_students_registration")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "academicRegistration", "Student with this registration already exists");
        }
        // --- PROJECTS ---
        if (isConstraint(constraintName, "projects_entity_id_name_key", "uq_projects_entity_name")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "name", "Project with this name already exists for this Entity");
        }
        if (isConstraint(constraintName, "attendances_qr_validation_hash_key", "uq_attendances_hash")) {
            return buildConflictResponse(SharedErrorCodes.DUPLICATED_RESOURCE_ERROR, "qrValidationHash", "Attendance already recorded (Duplicate QR Hash)");
        }
        if (constraintName.endsWith("_fkey")) {
            return buildDataIntegrityResponse(SharedErrorCodes.DATA_INTEGRITY_ERROR, "Cannot proceed because this resource is referenced by other records.");
        }
        return buildGenericDataIntegrityResponse(ex);
    }

    /**
     * Validates if the given constraint name matches either the raw Postgres constraint name or the custom
     *
     ...
     * @param customName   The expected custom constraint name (e.g., from @UniqueConstraint) to compare against.
     * @return true if the rawName matches either the postgresName or the customName (case-insensitive), false otherwise.
     */
    ...
    /**
     * Builds a conflict response for a duplicated resource error, using the provided error code and field name.
     *
     * <p>This method constructs a standardized API error response indicating that a resource with the specified
     * field value already exists, based on the provided error code which should correspond to a specific
     * duplication scenario. It uses internationalization to fetch the appropriate messages for both the
     * main error and the specific reason.
     *
     * @param code      The specific error code representing the duplication scenario (e.g., USER_ALREADY_EXISTS).
    ...
    /**
     * Builds a conflict response for a duplicated resource error, using the provided error code, field name, and fallback message.
     *
     * <p>This method constructs a standardized API error response indicating that a resource with the specified
     * field value already exists, based on the provided error code which should correspond to a specific
     * duplication scenario. It uses internationalization to fetch the appropriate messages for both the
     * main error and the specific reason. If the specific reason cannot be translated, it falls back to
     ...
     * @return A Response object with a conflict status and detailed error information.
     */
    private Response buildConflictResponse(GenericErrorCodes code, String fieldName, String fallbackMessage) {
    ...
    /**
     * Builds a conflict response for a data integrity violation, using the provided error code and message.
     *
     * <p>This method constructs a standardized API error response indicating that a data integrity violation
     * occurred, based on the provided error code which should correspond to a specific integrity issue. It
     * uses internationalization to fetch the appropriate messages for both the main error and the specific
     * reason.
     *
     * @param code    The specific error code representing the data integrity issue (e.g., DATA_INTEGRITY_ERROR).
    ...
    private Response buildGenericDataIntegrityResponse(ConstraintViolationException ex) {
        ...
        ApiError error = ApiError.of(SharedErrorCodes.DATA_INTEGRITY_ERROR.name(), msg, new Details(details));
        return Response.status(Response.Status.CONFLICT)
                .type(MediaType.APPLICATION_JSON_TYPE)
                .entity(ApiEnvelope.error(error))
                .build();
     ...
     *
     * <p>This method is used as a fallback for PersistenceExceptions that do not match known constraint
     * violations or data issues. It logs the exception for debugging purposes and returns a standardized
     * API error response indicating that an internal error occurred, without exposing sensitive details.
     *
