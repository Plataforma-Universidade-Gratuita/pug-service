...
public final class HibernateSearchUtils {
    ...
    /**
     * Search entities by name using Hibernate Search with default field names and minimum token length.
     *
        ...
        return s.search(type)
                .where(f ->
                        f.bool(b -> {
                            ...
                            b.should(f.wildcard().field(nameExactField).matching("*" + key + "*").boost(6f));
                            for (String t : tokens) {
                                if (t.length() >= minTokenLength) {
                                    b.should(f.wildcard().field(nameExactField).matching("*" + t + "*").boost(3f));
                                }
                            }
                            b.should(f.match().field(nameField).matching(key).fuzzy(1).boost(4f));
                            b.should(f.match().field(nameAutoField).matching(key).boost(2f));
                        }))
                .sort(f -> (nameSortField == null || nameSortField.isBlank())
                        ? f.score()
