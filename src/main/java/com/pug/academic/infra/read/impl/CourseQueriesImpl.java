 ...
 * <p>
 * This application-scoped bean handles the execution of read-only queries for courses.
 * It uses JPQL constructor expressions to implicitly join the course data with its
 * underlying School in a single database round-trip.
 */
@ApplicationScoped
...
public class CourseQueriesImpl implements CourseQueries {
   ...
   * {@inheritDoc}
   * <p>
   * To achieve a full-text search against the course name, this method first resolves
   * the matching courses via the search index, extracts their required School UUIDs,
   * fetches the corresponding schools, and finally assembles the complete {@link CourseView}.
   */
  @Override
