...
public class StudentProcessor {
    ...
    public static Student processUpdateInput(
        ...
        // We reconstruct the VO using existing values if the new value is null to support partial updates
        boolean hoursChanged = requiredHours != null || completedHours != null;
        if (hoursChanged) {
            BigDecimal newReq = requiredHours != null ? requiredHours : existingStudent.getCounterpartHours().getRequiredHours();
            BigDecimal newComp = completedHours != null ? completedHours : existingStudent.getCounterpartHours().getCompletedHours();
        ...
        if (periodChanged) {
            LocalDate newStart = startDate != null ? startDate : existingStudent.getPeriod().getStartDate();
