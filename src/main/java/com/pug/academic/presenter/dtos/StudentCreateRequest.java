 ...
 * academic enrollment details. It applies Jakarta Bean Validation constraints to ensure
 * all required components are structurally sound before attempting a complex, multi-domain transaction.
 *
 ...
 * @param email                the email address for the student's account (must be a valid format and max 254 characters)
 * @param password             the requested password for authentication (must not be blank, between 8 and 255 characters)
 * @param academicRegistration the university-issued academic registration string (must not be blank and max 15 characters)
 * @param campus               the designated university campus enum where the student is enrolled (must not be null)
 * @param courseId             the unique identifier (UUIDv7) of the enrolled course (must not be null)
 * @param requiredHours        the required counterpart hours the student must complete (must be non-negative)
