...
public record Period(LocalDate startDate, LocalDate dueDate) {
  ...
  public Period {
    ...
    if (startDate == null) {
      problems.add(
          new AppValidationException.Problem(AcademicErrorCodes.INVALID_PERIOD_BLANK, "startDate"));
    }
    if (dueDate == null) {
      problems.add(
          new AppValidationException.Problem(AcademicErrorCodes.INVALID_PERIOD_BLANK, "dueDate"));
    }

    if (startDate != null && dueDate != null) {
      if (dueDate.isBefore(startDate)) {
        problems.add(
            new AppValidationException.Problem(AcademicErrorCodes.INVALID_PERIOD_RANGE, "dueDate"));
      }
    }
