...
public record AcademicRegistration(String registration) {
   ...
   *
   * @param registration the registration string
   * @throws AppValidationException if the registration is null, blank, or too long. This exception
   *     may contain multiple validation problems.
   */
    ...
    if (StringUtils.isEmpty(registration)) {
      problems.add(
          new AppValidationException.Problem(
              AcademicErrorCodes.INVALID_REGISTRATION_BLANK, "registration"));
    } else {
      ...
      if (trimmedRegistration.length() > 15) {
        problems.add(
            new AppValidationException.Problem(
                AcademicErrorCodes.INVALID_REGISTRATION_LENGTH, "registration"));
      }
    }
