...
public enum GeoErrorCodes implements GenericCodes {
  ...
  /**
   * Indicates an attempt to create or update a city using an IBGE code
   * that is already registered to another city in the system.
   */
  CITY_ALREADY_EXISTS("error.domain.geo.city.already.exists"),

  /**
   * Indicates an attempt to modify or delete a protected system default city
   * (e.g., fixed university campus locations like Jaraguá do Sul or Joinville).
   */
  CITY_IS_DEFAULT("error.domain.geo.city.is.default"),

  /**
   * Indicates that a requested city could not be located in the underlying
   * data store by its ID or IBGE code.
   */
  CITY_NOT_FOUND("error.domain.geo.city.not.found"),

  /**
   * Indicates an attempt to delete a city that is currently assigned to
   * one or more active Partner entities, violating data retention policies.
   */
