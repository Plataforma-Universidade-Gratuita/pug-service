 ...
 * <p>
 * This class acts as an aggregate containing the city's unique identifier, name,
 * and its corresponding {@link IbgeCode}. It extends {@link DomainError} to
 * accumulate validation failures across its own fields and to bubble up errors
 * from its nested value objects.
 */
@Getter
@Value
...
public class City extends DomainError {
   ...
   * trims the provided name, and performs a full validation of the entity and its contents.
   *
   ...
   * <p>
   * Since this entity is immutable, this method returns a new {@code City} instance
   * with the updated, trimmed name if it differs from the current one. The new instance
   * is fully re-validated.
   *
   * @param newName the new name to assign to the city
   * @return a new, updated, and validated {@link City} instance, or {@code this} if the name is unchanged
   */
   ...
   * <p>
   * Returns a new, re-validated {@code City} instance with the updated code,
   * maintaining the immutability of the domain entity.
   *
   * @param newCode the new {@link IbgeCode} to assign to the city
   * @return a new, updated, and validated {@link City} instance, or {@code this} if the code is unchanged
   */
   ...
   *   <li>Validates the city name (inherited from {@link DomainError})</li>
   *   <li>Ensures the {@code ibgeCode} is not null (appends {@link GeoFieldErrorCodes#INVALID_IBGE_CODE_BLANK})</li>
   *   <li>Extracts and bubbles up any errors already present inside the {@link IbgeCode} value object</li>
   * </ul>
   */
