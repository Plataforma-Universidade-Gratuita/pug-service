 ...
 *
 * <p>This interface defines the contract for persisting, retrieving, updating, and deleting city
...
public interface CityRepository {
   ...
   *
   * <p><i>Note:</i> Certain default cities (e.g., Jaraguá do Sul and Joinville) are protected from
   * deletion by business rules and database constraints to maintain system integrity.
   *
   * @param id the unique identifier (UUIDv7) of the city to delete
   * @return {@code true} if the city was successfully deleted, {@code false} if the city was not
   *     found
   */
   ...
   *
   * <p>When a city is reconstituted from the persistence layer, it typically undergoes the same
   * domain validations as a newly created entity. Therefore, the returned {@link City} might
   * contain validation errors (verifiable via {@link City#hasFieldErrors()}) if the stored data
   ...
   * constraints before persisting a new city or updating an existing one's code.
   *
