 ...
 * operations for retrieving geographic data directly into lightweight {@link CityView}
...
public interface CityQueries {
   ...
   * @return an {@link Optional} containing the {@link CityView} if found, or {@link Optional#empty()} otherwise
   */
   ...
   * to provide fuzzy matching, accent-insensitivity, and autocomplete capabilities.
   *
   * @param key the raw search string or partial name provided by the user
   * @return a sorted {@link List} of {@link CityView} entries matching the search criteria,
