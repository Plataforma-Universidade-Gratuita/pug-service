 ...
 * pure Domain Aggregates and Value Objects.
 * <p>
...
public class CityProcessor {
   ...
   * <p>
   * This method translates the raw string representations into appropriate Value Objects
   * (e.g., {@link IbgeCode}) before passing them to the entity's factory method.
   * <p>
   * <b>Note:</b> The returned {@link City} object may contain accumulated domain validation
   * failures. The caller is responsible for checking {@link City#hasFieldErrors()} and
   * handling them appropriately (e.g., throwing an {@code AppValidationException}).
   *
   * @param name           the raw city name requested for creation
   * @param ibgeCodeString the raw 7-digit IBGE code string
   * @return a fully instantiated {@link City} domain aggregate, potentially containing validation errors
   */
   ...
   * returns a <i>new</i> instance of the {@link City} reflecting the applied changes.
   * The caller must verify {@link City#hasFieldErrors()} on the returned instance.
   *
   ...
   * @return a new {@link City} domain aggregate reflecting the requested updates, potentially containing validation errors
   */
