 ...
 * constraints, and coordinates with the persistence layer to ensure the integrity of geographic
 * data.
 */
public interface CityService {
   ...
   * validations (e.g., IBGE code formatting) are strictly applied before persistence.
   *
   ...
   * @throws AppValidationException if the input data violates domain constraints (e.g., blank name,
   *     malformed IBGE code)
   */
   ...
   *
   * <p>This operation enforces strict data retention and integrity policies. Attempting to delete
   * protected system defaults (e.g., Jaraguá do Sul or Joinville) or cities that are currently
   * assigned to active aggregates (such as Partner entities) is strictly prohibited and will
   * actively fail to prevent orphaned records.
   *
   * @param id the unique identifier (UUID) of the city to delete
   * @return {@code true} if the city was successfully deleted, {@code false} if the ID is null or
   *     if the actual database deletion was silently ignored (e.g., an idempotent concurrent
   *     delete)
   * @throws com.pug.shared.exceptions.ResourceNotFoundException if the city to be deleted does not
   *     exist
   * @throws com.pug.shared.exceptions.BusinessRuleException if the city is a protected default
   *     system record, or if it is currently referenced by other entities
   */
   ...
   * loading an aggregate to mutate it). For API responses, use {@link
   * CityReadService#getViewById(UUID)} instead.
   *
