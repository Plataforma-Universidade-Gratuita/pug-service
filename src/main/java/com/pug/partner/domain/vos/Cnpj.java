 ...
 * specific to Brazilian corporate taxpayer registry numbers without throwing immediate exceptions.
...
public class Cnpj extends DomainError {
   ...
   * <ul>
   *   <li>Must not be null or empty (appends {@link PartnerFieldErrorCodes#INVALID_CNPJ_BLANK})</li>
   *   <li>Must be exactly 14 digits long, cannot consist of the same repeated digit,
   *       and must pass the standard modulo-11 checksum algorithm for both verification digits
   *       (appends {@link PartnerFieldErrorCodes#INVALID_CNPJ_FORMAT})</li>
   * </ul>
   */
   ...
   * the last two digits (Verification Digits) of the CNPJ.
   *
  ...
  /**
   * Calculates a single CNPJ verification digit based on the modulo-11 algorithm and provided weights.
   *
   * @param cnpj    the numeric string containing the base digits for calculation
   * @param weights the specific array of integer multipliers required for the respective verification digit
