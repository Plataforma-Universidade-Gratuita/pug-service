...
public record Cnpj(String value) {
  ...
  public Cnpj {
    ...
    if (StringUtils.isEmpty(value)) {
      ...
      if (cleanedDigits.length() != 14) {
        problems.add(new AppValidationException.Problem(PartnerErrorCodes.INVALID_CNPJ_LENGTH, "cnpj"));
      } else {
        if (cleanedDigits.chars().distinct().count() == 1) {
          problems.add(new AppValidationException.Problem(PartnerErrorCodes.INVALID_CNPJ_FORMAT, "cnpj"));
        }
   ...
   * (Original method renamed to reflect it's only checksum)
   *
  ...
  /**
   * Calculates a CNPJ checksum digit.
   * (Original method renamed to be more descriptive)
   *
   * @param cnpj    the CNPJ string containing only digits
