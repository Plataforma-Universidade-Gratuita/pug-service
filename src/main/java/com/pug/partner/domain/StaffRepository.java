 ...
 *
 * <p>This interface defines the contract for assigning and revoking staff privileges that link an
 * authentication account directly to a partner organization (Entity). It abstracts the underlying
 * data storage mechanism to maintain a pure domain model.
 */
public interface StaffRepository {
   ...
   *
   * <p>This is typically used during cascading deletes when a partner entity is removed from the
   * system, ensuring no orphaned staff privileges remain.
   *
   ...
   *
   * <p>When a staff record is reconstituted from the persistence layer, it might contain validation
   * errors (verifiable via {@link Staff#hasFieldErrors()}) if the stored data is inconsistent with
   * current domain rules.
   *
   * @param accountId the unique identifier of the linked account
   * @return an {@link Optional} containing the {@link Staff} if found, or {@link Optional#empty()}
   *     if not
   */
  Optional<Staff> findOptionalByAccountId(UUID accountId);

  /**
   * Retrieves a list of all {@link Staff} aggregates currently linked to a specific partner entity.
   *
   * <p>Note: The returned objects may contain validation errors (verifiable via {@link
   * Staff#hasFieldErrors()}) if the stored data violates current domain rules.
   *
