 ...
 * <p>
 * This class exposes endpoints to assign, retrieve, and revoke organizational roles
 * for authentication accounts. It acts as the HTTP entry point, delegating queries to the
 * {@link StaffReadService} and commands to the {@link StaffService}, adhering to CQRS principles.
 */
@ApplicationScoped
@Path("/partners/staff")
...
public class StaffResource {
   ...
   * @return an HTTP 200 OK response containing an {@link ApiEnvelope} with the {@link StaffResponse}
   ...
   * the personal names of the staff. If omitted, it returns an unfiltered list of all staff.
   *
   * @param query the optional search query string
   * @return an HTTP 200 OK response containing an {@link ApiEnvelope} with a list of {@link StaffResponse}
   */
  @GET
   ...
   *
   * @param req the validated {@link StaffCreateRequest} containing the identity, credentials, and entity ID
   * @return an HTTP 201 Created response containing a {@code Location} header and the created {@link StaffResponse}
   * @throws DuplicateResourceException                      if an account with the email already exists or is already assigned to the entity
   * @throws com.pug.shared.exceptions.BusinessRuleException if the account is already assigned to a different entity
  ...
  /**
   * Permanently revokes staff privileges by deleting the assignment record and its associated account.
   *
