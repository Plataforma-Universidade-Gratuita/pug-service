...
public class QrValidationInfo extends DomainError {
    ...
    public static QrValidationInfo factory(
            ...
            BigDecimal longitude,
            String qrValidationHash) {
        QrValidationInfo vo = QrValidationInfo.builder()
                .duration(duration)
                .latitude(latitude)
                .longitude(longitude)
    ...
    private void validate() {
        ...
        if (hasLat) {
            if (latitude.compareTo(BigDecimal.valueOf(90)) > 0 || latitude.compareTo(BigDecimal.valueOf(-90)) < 0) {
                addError(new AppValidationException.Problem(ProjectsErrorCodes.INVALID_ATTENDANCE_GEO_INVALID_LAT));
            }
        }

        if (hasLon) {
            if (longitude.compareTo(BigDecimal.valueOf(180)) > 0 || longitude.compareTo(BigDecimal.valueOf(-180)) < 0) {
                addError(new AppValidationException.Problem(ProjectsErrorCodes.INVALID_ATTENDANCE_GEO_INVALID_LONG));
            }
        }
    }
}