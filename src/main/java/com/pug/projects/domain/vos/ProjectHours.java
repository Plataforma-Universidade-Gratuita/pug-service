...
public class ProjectHours extends DomainError {
    ...
    public static ProjectHours factory(BigDecimal offeredHours, BigDecimal completedHours) {
        ...
        ProjectHours vo = ProjectHours.builder()
    ...
    private void validate() {
        ...
        if (completedHours.signum() < 0) {
            addError(new AppValidationException.Problem(ProjectsErrorCodes.INVALID_COMPLETED_HOURS_NEGATIVE));
        }

        if (completedHours.compareTo(offeredHours) > 0) {
            addError(new AppValidationException.Problem(ProjectsErrorCodes.INVALID_COMPLETED_HOURS_EXCEEDS));
        }
    }
}