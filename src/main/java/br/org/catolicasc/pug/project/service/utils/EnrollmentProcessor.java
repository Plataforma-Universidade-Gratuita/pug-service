...
public final class EnrollmentProcessor {
   ...
   *
   * <p>This method expects fully reconstituted {@link FormerStudent} and {@link Project}
   * aggregates, and delegates the initialization of the enrollment to {@link
   * Enrollment#factory(FormerStudent, Project)}. The returned instance is immediately
   * self-validated; any accumulated domain validation problems can be inspected via {@link
   * Enrollment#hasFieldErrors()} and {@link Enrollment#getFieldErrors()} and should be translated
   * into an {@link AppValidationException} by the calling service when appropriate.
   *
