 ...
 * new enrollment aggregate, namely the identifier of the target project. The formerStudent account to be
 * enrolled is resolved server-side from the current authentication context (for example, via {@code
 * AuthService.getCurrentAccountId()}), ensuring that a formerStudent can only create enrollments on their
 * own behalf.
 *
